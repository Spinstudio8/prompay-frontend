...
const Profile = () => {
  ...
  const { token, adminViewUserProfile, tokenPayload } = useSelector(
    (state) => state.user
  );

  const [userProfile, setUserProfile] = useState(adminViewUserProfile);
  const [fullName, setFullName] = useState(
    `${adminViewUserProfile.firstName} ${adminViewUserProfile.lastName}`
  );
  ...
  const updateProfile = async () => {
    ...
    try {
      ...
      dispatch(setAdminViewUserProfile(data));
      ...
      <DashboardLayout>
        <div className='text-black dark:bg-main-dark-bg'>
          <div className='pt-[110px] md:pt-[46px] mx-[15px] md:mx-[50px]'>
            ...
            >
              {!adminViewUserProfile || userProfileLoading ? (
                <Loader />
              ) : (
                <div className='w-full'>
                  <div className='h-[80px] md:h-[140px] bg-primaryGreen rounded-t-lg'></div>
                  <div className='relative dark:text-black bg-white rounded-b-lg shadow-md px-4 pb-5 mb-2'>
